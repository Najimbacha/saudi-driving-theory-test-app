...
export default function Flashcards() {
  ...
  if (showResult) {
    ...
    return (
      <div className="min-h-screen bg-background">
        <header className="bg-gradient-hero text-primary-foreground dark:text-foreground p-4 pb-6">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-destructive/10 rounded-xl p-4 text-center">
              ...
              <p className="text-2xl font-bold text-destructive">{sessionStats.incorrect}</p>
          ...
          <div className="bg-card rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-success">{masteryStats.mastered}/{masteryStats.total}</span>
              </div>
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
          ...
          <div className="bg-card rounded-xl p-4 w-full max-w-xs space-y-2 mt-4">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">{t('flashcards.mastered')}</span>
              <span className="font-medium text-success">{masteryStats.mastered}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">{t('flashcards.reviewing')}</span>
              <span className="font-medium text-info">{masteryStats.reviewing}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">{t('flashcards.learning')}</span>
              <span className="font-medium text-warning">{masteryStats.learning}</span>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="bg-gradient-hero text-primary-foreground dark:text-foreground p-4">
          ...
          {isFlipped && (
            ...
            >
              <Button
                ...
                variant="outline"
                size="lg"
                className="flex-1 max-w-[140px] h-14 border-destructive/40 hover:bg-destructive/10 hover:border-destructive/70"
              >
                <X className="w-6 h-6 text-destructive mr-2" />
                <span className="text-destructive">{t('flashcards.again')}</span>
              </Button>
              <Button
                onClick={() => handleResponse(true)}
                size="lg"
                className="flex-1 max-w-[140px] h-14 bg-success text-success-foreground hover:bg-success/90"
              >
                <Check className="w-6 h-6 mr-2 text-success-foreground" />
                {t('flashcards.gotIt')}
              </Button>
            </motion.div>
          )}
